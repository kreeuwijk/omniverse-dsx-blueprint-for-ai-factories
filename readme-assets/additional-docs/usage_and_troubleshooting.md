...
#### Streaming app exits with code -11 (SIGSEGV)
When running `./run_streaming.sh`, the Kit streaming application may exit with code -11 (segmentation fault) in some environments—for example, headless servers without a display or with certain GPU/driver configurations. The **build** completes successfully; the failure occurs at launch. If you see this, try: (1) running on a machine with an NVIDIA RTX GPU and supported driver (see README Prerequisites), (2) ensuring a display or virtual display is available if required by your setup, or (3) using the non-streaming app for local testing: `./repo.sh launch dsx.kit` (opens a window). For CI or build-only workflows, run the [build-from-fresh-checkout](../README.md#building-from-a-fresh-checkout) steps and skip launch.
...
Running `./repo.sh build` (or `.\repo.bat build`) from a clean clone handles everything automatically: submodule initialization, kit-cae schema/extension build, extension precaching, and DSX compilation. See the [Building from a fresh checkout](../../README.md#building-from-a-fresh-checkout) section in the main README.
