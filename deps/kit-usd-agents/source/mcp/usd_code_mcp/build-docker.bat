...
@echo off
REM Build script for USD Code MCP Docker container
REM
...
set "AIQ_DIR=%ROOT_DIR%aiq\usd_code_fns"
...
echo Building AIQ functions package (usd_code_fns)...
pushd "%AIQ_DIR%"
if %errorlevel% neq 0 (
...
echo Building MCP Python package (usd_code_mcp)...
poetry build
if %errorlevel% neq 0 (
...
dir /b dist\usd_code_aiq-*.whl >nul 2>&1
if %errorlevel% neq 0 (
    echo ERROR: AIQ wheel not found in dist\
    exit /b 1
)

dir /b dist\usd_code_mcp-*.whl >nul 2>&1
if %errorlevel% neq 0 (
