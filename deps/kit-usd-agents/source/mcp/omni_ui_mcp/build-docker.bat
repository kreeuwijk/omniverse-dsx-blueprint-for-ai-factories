...
set "AIQ_DIR=%ROOT_DIR%aiq\omni_ui_fns"
...
echo Building AIQ functions package (omni_ui_fns)...
pushd "%AIQ_DIR%"
if %errorlevel% neq 0 (
...
REM Build the MCP package using Poetry (named omni-ui-aiq)
echo Building MCP Python package (omni_ui_aiq)...
poetry build
if %errorlevel% neq 0 (
...
dir /b dist\omni_ui_fns-*.whl >nul 2>&1
if %errorlevel% neq 0 (
    echo ERROR: AIQ wheel not found in dist\
    exit /b 1
)

dir /b dist\omni_ui_aiq-*.whl >nul 2>&1
if %errorlevel% neq 0 (
