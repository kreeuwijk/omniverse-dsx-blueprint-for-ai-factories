...
"""Get USD modules function implementation."""
...
async def get_usd_modules() -> Dict[str, Any]:
    """Return a list of all USD modules from the USD Atlas data.

    Returns:
        Dictionary containing:
        - success: bool indicating if the operation succeeded
        - result: A JSON string containing all USD module information
    ...
    try:
        ...
        result = _usd_atlas_service.get_modules()
        result_json = json.dumps(result, indent=2)

        logger.info("Successfully retrieved USD modules")
        return {"success": True, "result": result_json, "error": None}

    except Exception as e:
        error_msg = f"Error retrieving USD modules: {str(e)}"
        logger.error(error_msg)
        return {"success": False, "error": error_msg, "result": ""}
