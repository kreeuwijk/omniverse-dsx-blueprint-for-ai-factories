...
"""Get USD module detail function implementation."""
...
async def get_usd_module_detail(module_names: str) -> Dict[str, Any]:
    """Return detailed information about specific USD modules, including their classes and functions.

    Args:
        module_names: Comma-separated list of module names (e.g., "pxr.Usd,pxr.UsdGeom" or "Usd,UsdGeom")

    Returns:
        Dictionary containing:
        - success: bool indicating if the operation succeeded
        - result: JSON string with detailed information about the modules including classes and functions
    ...
    try:
        ...
        # Parse the comma-separated module names
        module_list = [name.strip() for name in module_names.split(",") if name.strip()]

        if not module_list:
            error_msg = "No valid module names provided"
            logger.error(error_msg)
            return {"success": False, "error": error_msg, "result": ""}
        ...
        for module_name in module_list:
            try:
                result = _usd_atlas_service.get_module_detail(module_name)
                results[module_name] = result
            except Exception as e:
                results[module_name] = {"error": f"Failed to retrieve details: {str(e)}"}

        result_json = json.dumps(results, indent=2)

        logger.info(f"Successfully retrieved details for {len(module_list)} modules")
