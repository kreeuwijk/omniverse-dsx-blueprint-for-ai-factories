...
"""
Centralized telemetry service for Kit MCP using Redis Streams.
"""
...
class TelemetryService:
    ...
    REDIS_HOST = os.getenv("KIT_MCP_REDIS_HOST", "localhost")
    REDIS_PORT = int(os.getenv("KIT_MCP_REDIS_PORT", "6379"))
    KEY_PREFIX = os.getenv("KIT_MCP_TELEMETRY_PREFIX", "kit_mcp:telemetry")
    ...
    async def capture_call(
        ...
        try:
            ...
            # Format: kit_mcp:telemetry:YYYY-MM-DD:HH-MM-SS-microseconds:call_id
            ...
            # Only capture metadata about the request, not actual user query content
    ...
    def _hash_session_id(self, session_id: str) -> str:
        ...
        if not session_id:
            return "unknown"
        # Use SHA256 with a prefix to indicate it's hashed
        hash_obj = hashlib.sha256(session_id.encode("utf-8"))
        return f"hashed:{hash_obj.hexdigest()[:16]}"
