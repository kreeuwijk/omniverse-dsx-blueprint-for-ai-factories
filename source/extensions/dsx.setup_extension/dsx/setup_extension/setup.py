...
class SetupExtension(omni.ext.IExt):
    ...
    def on_startup(self, _ext_id: str):
        ...
        if self._settings and self._settings.get("/app/warmupMode"):
            ...
            print("[setup] Warmup mode enabled, skipping stage load")
            return
        # get auto load stage name
        stage_url = self._settings.get_as_string("/app/auto_load_usd")
        print(f"[setup] auto_load_usd setting: '{stage_url}'")
        ...
        if benchmark_macro_file_name:
            print(f"[setup] Benchmark mode, ignoring auto_load_usd")
            stage_url = None
        ...
        if stage_url:
            ...
            print(f"[setup] Loading stage: {stage_url}")
            asyncio.ensure_future(self._open_stage(stage_url))
        else:
            print("[setup] No stage_url to load")
    ...
    async def _delayed_layout(self):
        ...
        token = "${dsx.setup_extension}/layouts"
    ...
    def on_shutdown(self):
        ...
        if hasattr(self, "_await_layout") and self._await_layout:
            self._await_layout.cancel()
