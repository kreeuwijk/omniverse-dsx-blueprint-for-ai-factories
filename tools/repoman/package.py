...
def package_container(options: argparse.Namespace, config: dict, build_path: pathlib.Path):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        # Copy the entire assets folder into the Docker build context
        assets_src = pathlib.Path("assets")
        assets_dst = pathlib.Path(tmpdir + "/assets")
        if assets_src.exists() and assets_src.is_dir():
            shutil.copytree(assets_src, assets_dst, dirs_exist_ok=True)
        else:
            console.print("Warning: assets folder not found, skipping asset copy.", style=WARN_COLOR)
